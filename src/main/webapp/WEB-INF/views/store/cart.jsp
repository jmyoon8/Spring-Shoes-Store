...
<c:if test="${result==1}">
	<script type="text/javascript">
		alert("장바구니에서 삭제했습니다.");
		location.href="/springStore/member/cart
	</script>
</c:if>
<c:if test="${cResult==0}">
	<script type="text/javascript">
		alert("주문실패했습니다. 잠시후 다시 시도해주세요!");
	</script>
</c:if>
<c:if test="${cResult==1}">
	<script type="text/javascript">
...
<div id="contain">
	...
	<div class="in" id="contents" >
		<form action="" method="post" name="cartForm" onsubmit="">
				...
				<tbody>
				  <c:if test="${total>0}">
					<c:forEach var="vo" items="${vo}">
						<tr style="text-align: center;">
							<td><input type="checkbox" name="box" class="re" value="${vo.cNum}"></td>
				...
				<tfoot>
					<tr>
						...
						<td colspan="6" align="right">
							<input type="button" value="구매확정" onclick="confirmCart();">
							<input type="button" value="삭제" onclick="deleteCart();">
							<input type="button" value="장바구니 수정" onclick="window.open('${path}/member/modifyCart','장바구니 수정','menubar=no width=500 height=600');"></td>
					</tr>
				</tfoot>
			</table>
		</form>
